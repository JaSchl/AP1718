...
x, y = np.genfromtxt('werte31.txt' , unpack=True)
...
L1plot = np.linspace(0, 0.35) #Grenzen fuer linregr. guckt nach den Grenzwerten fuer die x-Werte.
plt.title('Zylindrischer Stab, beidseitige Einspannung 1') #Ueberschrift, die ueber dem PLot stehen wird
...
plt.xlabel(r"$(3L^2x-4x^3)/10^{-3} \,\, in \,\, m^3$")
...
plt.savefig('plot31.pdf') #erstellt und speichert automatisch den Plot als pdf datei
